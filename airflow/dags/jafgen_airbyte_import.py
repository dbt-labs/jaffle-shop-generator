...
def validate_dag_config(**context):
    ...
    if not manifest_url:
        raise AirflowException(
            "manifest_url is required in DAG run configuration. Example: "
    ...
    if not api_name:
        raise AirflowException(
            "api_name is required in DAG run configuration. Provide a name "
            "for the imported API (e.g., 'shopify', 'salesforce')"
        )

    # Validate API name format
    if not api_name.replace("_", "").replace("-", "").isalnum():
        raise AirflowException(
            f"api_name '{api_name}' must contain only alphanumeric "
...
def download_airbyte_manifest(**context):
    ...
    try:
        ...
        if manifest_size < 100:
            raise AirflowException(
                f"Manifest file too small ({manifest_size} bytes), likely invalid"
            )

        # Check for basic YAML structure
        if "streams:" not in response.text and "entities:" not in response.text:
            logging.warning(
                "Manifest may not contain expected stream/entity definitions"
            )
        ...
        raise AirflowException(
            f"Failed to download manifest from {manifest_url}: {str(e)}"
        )
...
def import_airbyte_schemas(**context):
    ...
    try:
        ...
        import_result = {
            ...
            "entity_summary": {
                schema.name: list(schema.entities.keys()) for schema in schemas
            },
        }

        logging.info(
            f"Import successful: {len(schemas)} schemas, {total_entities} "
            f"entities, {total_attributes} attributes"
        )
...
def generate_imported_data(**context):
    ...
    try:
        ...
        logging.info(
            f"Generation complete: {generation_stats['total_records']} total records"
        )
        ...
        raise AirflowException(
            f"Failed to generate data from imported schemas: {str(e)}"
        )
...
def validate_imported_data(**context):
    ...
    try:
                    ...
                    try:
                        ...
                        if record_count == 0:
                            validation_results["errors"].append(
                                f"{entity_name}: No records generated"
                            )

                        if csv_file.stat().st_size == 0:
                            validation_results["errors"].append(
                                f"{entity_name}: Empty file"
                            )
        ...
        logging.info(
            f"Validation passed: "
            f"{len(validation_results['entities_validated'])} entities, "
...
def cleanup_temp_files(**context):
    ...
    cleanup_results = {"files_removed": [], "errors": []}
    ...
    try:
        ...
        logging.info(
            f"Cleanup complete: "
            f"{len(cleanup_results['files_removed'])} files removed"
        )
