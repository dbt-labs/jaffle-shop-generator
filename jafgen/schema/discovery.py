...
from .exceptions import CircularDependencyError, SchemaLoadError
from .interfaces import SchemaLoader
from .models import SystemSchema, ValidationError, ValidationResult, ValidationWarning
...
class SchemaDiscoveryEngine:
    ...
    def discover_and_load_schemas(
        self, schema_dir: Path
    ) -> Tuple[List[SystemSchema], ValidationResult]:
        ...
        if not schemas:
            return [], ValidationResult(is_valid=False, errors=load_errors)
        ...
        validation_result.is_valid = (
            validation_result.is_valid and len(load_errors) == 0
        )

        return schemas, validation_result

    def _validate_schemas_individually(
    ...
    def get_schema_summary(
        ...
        for schema in schemas:
            ...
            total_attributes = sum(
                len(entity.attributes) for entity in schema.entities.values()
            )
                ...
                for attr_config in entity_config.attributes.values():
                    if attr_config.link_to:
                        ...
                        if len(parts) >= 2:
                            ...
                            if (
                                target_entity in global_entities
                                and target_entity != full_entity_name
                            ):
    ...
    def get_generation_order(self, schemas: List[SystemSchema]) -> List[str]:
        ...
        if len(result) != len(dependency_graph):
            ...
            raise CircularDependencyError(
                f"Circular dependencies detected among entities: {remaining}"
            )
