    ...
    def generate_system(self, schema: SystemSchema) -> GeneratedSystem:
        ...
        try:
            ...
            if self.link_resolver:
                ...
                schema_entity_keys = [f"{schema.name}.{name}" for name in schema.entities.keys()]
                ordered_entity_keys = [key for key in generation_order if key in schema_entity_keys]
    ...
    def generate_multiple_systems(self, schemas: List[SystemSchema]) -> List[GeneratedSystem]:
        ...
        try:
            ...
            for entity_key in generation_order:
                ...
                self.link_resolver.register_entity(schema_name, entity_name, entity_data)
            ...
            for schema in schemas:
                if schema.name in generated_systems:
                    ...
                    # Calculate metadata
                    total_records = sum(len(data) for data in generated_system.entities.values())
                    entity_counts = {name: len(data) for name, data in generated_system.entities.items()}
                    ...
                    result_systems.append(GeneratedSystem(
                        schema=schema,
                        entities={},
                        metadata=GenerationMetadata(
                            generated_at=datetime.now(),
            ...
            raise GenerationError(f"Failed to generate multiple systems: {str(e)}") from e
