...
from jafgen.schema import SchemaDiscoveryEngine, YAMLSchemaLoader, CircularDependencyError
...
"""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(schemas) == 2  # Both should load, but validation will catch issues
...
"""
        ...
        schema = SystemSchema(
            ...
            entities={
                ...
                "orders": EntityConfig(
                    ...
                    attributes={
                        "id": AttributeConfig(type="uuid", unique=True),
                        "user_id": AttributeConfig(type="link", link_to="test-schema.users.id")
                    }
                )
            }
        )
    ...
    def test_validate_schema_compatibility(self):
        ...
        assert any(warning.type == "conflicting_output_path" for warning in result.warnings)
        ...
        schema = SystemSchema(
            ...
            entities={
                "entity_a": EntityConfig(
                    ...
                    attributes={
                        "id": AttributeConfig(type="uuid", unique=True),
                        "b_ref": AttributeConfig(type="link", link_to="circular.entity_b.id")
                    }
                ),
                "entity_b": EntityConfig(
                    ...
                    attributes={
                        "id": AttributeConfig(type="uuid", unique=True),
                        "a_ref": AttributeConfig(type="link", link_to="circular.entity_a.id")
                    }
                )
            }
        )
