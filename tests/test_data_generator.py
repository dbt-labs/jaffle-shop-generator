...
class TestDataGenerator:
    ...
    def test_generate_entity_multiple_records(self):
        ...
        mimesis_engine.generate_value.side_effect = [
            "John Doe",
            "john@example.com",  # Record 1
            "Jane Smith",
            "jane@example.com",  # Record 2
        ]
        ...
        entity_config = EntityConfig(
            ...
            attributes={
                ...
                "user_id": AttributeConfig(
                    type="uuid", required=True, link_to="users.id"
                ),
            },
        )
    ...
    def test_generate_entity_link_resolution_failure_non_required(self):
        ...
        entity_config = EntityConfig(
            ...
            attributes={
                "name": AttributeConfig(type="text", required=True),
                "user_id": AttributeConfig(
                    type="uuid", required=False, link_to="users.id"
                ),
            },
        )
    ...
    def test_generate_system_with_dependencies(self):
        ...
        dependency_graph.get_generation_order.return_value = [
            "test_system.users",
            "test_system.orders",
        ]
        ...
        link_resolver.register_entity.assert_any_call(
            "test_system", "users", result.entities["users"]
        )
        link_resolver.register_entity.assert_any_call(
            "test_system", "orders", result.entities["orders"]
        )
