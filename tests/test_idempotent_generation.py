...
class TestIdempotentGeneration:
                ...
                "orders": EntityConfig(
                    ...
                    attributes={
                        ...
                        "user_id": AttributeConfig(
                            type="link", link_to="test-system.users.id", required=True
                        ),
                        "amount": AttributeConfig(
                            type="float",
                            constraints={"min_value": 10.0, "max_value": 500.0},
                            required=True,
                        ),
                        "created_at": AttributeConfig(
                            type="datetime.datetime", required=True
                        ),
                    },
                ),
            },
    ...
    def test_different_generation_different_seed(
        self, sample_schema: SystemSchema, output_manager: OutputManager
    ):
    ...
    def test_metadata_persistence(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            original_metadata = output_manager.write_system_data(
                generated_system, output_path
            )
    ...
    def test_reproducibility_verification(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert not output_manager.verify_reproducibility(
                output_path, different_metadata
            )
...
class TestMultipleSchemaIdempotency:
            ...
            entities={
                "orders": EntityConfig(
                    ...
                    attributes={
                        "id": AttributeConfig(type="uuid", unique=True, required=True),
                        "user_id": AttributeConfig(
                            type="link", link_to="users-schema.users.id", required=True
                        ),
                        "amount": AttributeConfig(
                            type="float",
                            constraints={"min_value": 10.0, "max_value": 500.0},
    ...
    def test_multiple_schema_idempotency(self, linked_schemas: List[SystemSchema]):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_writers = {"csv": CSVWriter(), "json": JSONWriter()}
            output_manager = OutputManager(output_writers)

            # Generate data first time
            generated_systems1 = data_generator.generate_multiple_systems(
                linked_schemas
            )
            ...
            generated_systems2 = data_generator2.generate_multiple_systems(
                linked_schemas
            )
