...
class TestSchemaDrivenOutput:
    ...
    def basic_schema(self) -> SystemSchema:
        ...
        return SystemSchema(
            ...
            version="1.0.0",
            seed=42,
            output=OutputConfig(format=["csv", "json"], path="."),
            entities={
                "users": EntityConfig(
                    name="users",
                    count=5,
                    attributes={
                        ...
                        "email": AttributeConfig(
                            type="person.email", unique=True, required=True
                        ),
                    },
                )
            },
    ...
    def test_custom_output_path(
        self, data_generator: DataGenerator, output_manager: OutputManager
    ):
        ...
        schema = SystemSchema(
            ...
            version="1.0.0",
            seed=42,
            output=OutputConfig(format=["csv"], path="custom/output/directory"),
            entities={
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            custom_file = (
                output_path / "custom" / "output" / "directory" / "products.csv"
            )
    ...
    def test_per_entity_output_configuration(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            orders_duckdb = (
                output_path / "generated_data.duckdb"
            )  # DuckDB creates a database file
    ...
    def test_format_specific_options(
        ...
        schema = SystemSchema(
            ...
            output=OutputConfig(
                ...
                path=".",
                formats={
                    "csv": FormatConfig(type="csv", options={"encoding": "utf-8"}),
                    "json": FormatConfig(
                        type="json", options={"indent": 4, "ensure_ascii": False}
                    ),
                },
            ),
            entities={
    ...
    def test_custom_filename_patterns(
        ...
        schema = SystemSchema(
            ...
            output=OutputConfig(
                ...
                formats={
                    "csv": FormatConfig(
                        type="csv", filename_pattern="{entity_name}_data.{ext}"
                    ),
                    "json": FormatConfig(
                        type="json", filename_pattern="export_{entity_name}.{ext}"
                    ),
                },
            ),
            entities={
                    ...
                    "special_items": EntityOutputConfig(
                        formats={
                            "json": FormatConfig(
                                ...
                                options={
                                    "indent": 8
                                },  # Override with 8-space indentation
    ...
    def test_mixed_configuration_inheritance(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            users_json = (
                ...
                / "default_output"
                / "users_special"
                / "user_export_users.json"
            )
            users_parquet = (
                output_path / "default_output" / "users_special" / "users.parquet"
            )
            assert users_json.exists()
            assert users_parquet.exists()
    ...
    def test_output_format_validation(
        ...
        schema = SystemSchema(
            ...
            version="1.0.0",
            seed=42,
            output=OutputConfig(format=["csv", "unsupported_format"], path="."),
            entities={
    ...
    def test_idempotency_with_schema_configuration(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            json_file = (
                output_path / "custom_items" / "items.json"
            )  # JSON also goes to custom path
