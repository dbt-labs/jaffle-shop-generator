...
class TestAirbyteTranslator:
    ...
    def test_type_mappings(self):
        ...
        assert (
            translator._map_json_schema_type("string", "date-time", {})
            == "datetime.datetime"
        )
        assert translator._map_json_schema_type("string", "date", {}) == "datetime.date"
        assert (
            translator._map_json_schema_type("string", "uuid", {})
            == "cryptographic.uuid"
        )
        ...
        # Test boolean type
        assert (
            translator._map_json_schema_type("boolean", None, {})
            == "development.boolean"
        )
    ...
    def test_unsupported_type_warnings(self):
        ...
        assert any(
            "Array types not supported" in w.message for w in translator.warnings
        )
        ...
        assert len(translator.warnings) > 0
        assert any(
            "Object types not supported" in w.message for w in translator.warnings
        )
    ...
    def test_enum_handling(self):
        ...
        assert len(translator.warnings) > 0
        assert any(
            "Mixed-type enum not fully supported" in w.message
    ...
    def test_constraint_extraction(self):
        ...
        field_schema = {"minLength": 5, "maxLength": 50, "pattern": "^[A-Z]+$"}
        constraints = translator._extract_constraints(field_schema)
        ...
        field_schema = {"minItems": 1, "maxItems": 10}
        constraints = translator._extract_constraints(field_schema)
    ...
    def test_unique_field_detection(self):
        ...
        # Test ID fields
        attr_config = translator._convert_json_schema_field(
            "id", {"type": "string"}, True
        )
        assert attr_config.unique is True

        attr_config = translator._convert_json_schema_field(
            "uuid", {"type": "string"}, True
        )
        assert attr_config.unique is True

        attr_config = translator._convert_json_schema_field(
            "user_id", {"type": "string"}, True
        )
        assert attr_config.unique is True

        # Test UUID format
        attr_config = translator._convert_json_schema_field(
            "identifier", {"type": "string", "format": "uuid"}, True
        )
        assert attr_config.unique is True
    ...
    def test_invalid_manifest_structure(self, tmp_path: Path):
        ...
        with pytest.raises(
            ValueError, match="Manifest file must contain a YAML object"
        ):
            translator.translate_manifest(manifest_file)
    ...
    def test_stream_without_name_error(self, tmp_path: Path):
        ...
        assert any(
            "Stream must have a name" in e.message for e in validation_result.errors
        )
    ...
    def test_stream_without_schema_error(self, tmp_path: Path):
        ...
        manifest_data = {"version": "0.2.0", "streams": [{"name": "test_stream"}]}
        ...
        assert len(validation_result.errors) > 0
        assert any(
            "must have a json_schema" in e.message for e in validation_result.errors
        )
    ...
    def test_validation_result(self):
        ...
        translator.warnings.append(
            ValidationWarning(type="test_warning", message="Test warning message")
        )
        translator.errors.append(
            ValidationError(type="test_error", message="Test error message")
        )
